...
namespace Videoficha.Views
{
    public partial class SystemInfoEditWindow : Window
    {
        ...
        public SystemInfoEditWindow(List<string> systemInfo, string filePath)
        {
            ...
            // Cargar los valores actuales en los TextBox con validación
            if (_systemInfo != null && _systemInfo.Count >= 6)
            {
            ...
            {
                // Si la lista no tiene suficientes elementos, inicializar con vacíos para evitar crash
                ModelTextBox.Text = "";
                OSTextBox.Text = "";
                ProcessorTextBox.Text = "";
                RAMTextBox.Text = "";
                StorageTextBox.Text = "";
                GraphicsTextBox.Text = "";
                
                // Asegurar que la lista tenga al menos 6 elementos para poder guardar después
                if (_systemInfo == null) _systemInfo = new List<string>();
                while (_systemInfo.Count < 6) _systemInfo.Add("");
            }
